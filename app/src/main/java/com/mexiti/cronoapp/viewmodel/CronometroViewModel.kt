...
class CronometroViewModel @Inject constructor(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            repository.getCronoByID(id).collect{
                ...
                if(item != null){
                    ...
                    Log.d("Error", "El objeto creado es nulo")
                }
            }
        }
    }
    ...
    fun pausar(){
        state = state.copy(cronometroActivo = false, showSaveButton = true)
    }

    fun detener(){
        cronoJob?.cancel()
        time = 0
        state = state.copy(cronometroActivo = false, showSaveButton = false, showTextField = false)
    }
